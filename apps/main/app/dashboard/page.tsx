...
export default function Dashboard() {
  ...
  return (
    <DashboardShell projects={projects} projectId={projectId} flagCount={flags.length} onProjectChange={selectProject}>
      <div className="dashboard-inner">
        {projectsQuery.isLoading ? <div className="project-empty-state"><p>Loading projects…</p></div> : projects.length === 0 ? <ProjectEmptyState newProjectName={newProjectName} setNewProjectName={setNewProjectName} onSubmit={submitCreateProject} isPending={createProject.isPending} /> : <>
          ...
          {alerts && <div className={`dashboard-alert ${error || projectsQuery.error || flagsQuery.error ? "is-error" : "is-success"}`} role="status"><span>{error || projectsQuery.error?.message || flagsQuery.error?.message || notice}</span><button type="button" onClick={() => { setError(""); setNotice(""); }} aria-label="Dismiss message">×</button></div>}

          <div className="dashboard-grid" id="flags">
            <section className="flags-panel">
              <div className="panel-heading"><div><h2>Feature flags</h2><p>Manage and monitor your releases.</p></div><Link href="/flags" className="panel-link">View all <ArrowUpRight /></Link></div>
              ...
              <FlagTable flags={flags} visibleFlags={visibleFlags} selected={selected} setSelected={setSelected} isLoading={flagsQuery.isLoading} />
            </section>
...
function ProjectEmptyState({ newProjectName, setNewProjectName, onSubmit, isPending }: { newProjectName: string; setNewProjectName: (value: string) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; isPending: boolean }) {
  return <section className="project-empty-state"><div className="project-empty-icon"><Flag /></div><h1>Create your first project</h1><p>Projects keep your feature flags organized. Create one to get started.</p><form onSubmit={onSubmit}><Input aria-label="Project name" placeholder="Project name" value={newProjectName} onChange={(event: ChangeEvent<HTMLInputElement>) => setNewProjectName(event.target.value)} required /><Button className="button-primary" type="submit" disabled={isPending}>{isPending ? "Creating…" : "Create project"}</Button></form></section>;
}

function FlagTable({ flags, visibleFlags, selected, setSelected, isLoading }: { flags: FlagRecord[]; visibleFlags: FlagRecord[]; selected: number; setSelected: (index: number) => void; isLoading: boolean }) {
  return <div className="flags-table"><div className="flag-row table-heading"><span>Flag</span><span>Status</span><span>Updated</span><span /></div>{isLoading ? <div className="table-empty">Loading flags…</div> : visibleFlags.length === 0 ? <div className="table-empty">No flags yet. Create your first flag to get started.</div> : visibleFlags.map((flag) => { const originalIndex = flags.indexOf(flag); return <button type="button" className={`flag-row ${selected === originalIndex ? "selected" : ""}`} key={flag.id} onClick={() => setSelected(originalIndex)}><span className="flag-name"><i className={`status-dot ${flag.enabled ? "green" : "purple"}`} /><span><b>{flag.key}</b><small>{flag.description ?? flag.name}</small></span></span><span><Badge variant={flag.enabled ? "default" : "secondary"} className={`rollout-pill ${flag.enabled ? "on" : "off"}`}><i />{flag.enabled ? "Enabled" : "Off"}</Badge></span><span className="updated">{formatUpdated(flag.updatedAt)}</span><span className="row-arrow"><ArrowUpRight /></span></button>; })}</div>;
}
