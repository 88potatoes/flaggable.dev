...
import { DrizzleValueSchemaRepository } from "./repo";
...
describe("DrizzleValueSchemaRepository", () => {
  test("lists by project", async () => {
    const { db, query } = createDatabase();
    await new DrizzleValueSchemaRepository(db).listByProject({ projectId: "project-1" });
  ...
  test("finds by id", async () => {
    const { db, query } = createDatabase();
    await new DrizzleValueSchemaRepository(db).findById({ schemaId: "schema-1" });
    expect(query.where).toHaveBeenCalled();
  ...
  test("creates and updates records", async () => {
    const { db, query } = createDatabase();
    const record = { id: "schema-1", projectId: "project-1", name: "Text" } as never;
    await new DrizzleValueSchemaRepository(db).create({ record });
    await new DrizzleValueSchemaRepository(db).update({
      schemaId: "schema-1",
      values: { name: "Updated" },
    });
